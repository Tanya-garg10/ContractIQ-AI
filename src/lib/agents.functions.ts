    ...
    const aiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        contents: [
          {
            parts: [
              { text: `${SYSTEM_PROMPT}\n\nAnalyze the attached contract titled "${filename}" and return the JSON described in the system prompt. Be thorough but return valid JSON only.` },
              mediaBlock,
            ],
          },
        ],
...
export const startAnalysis = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const aiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [
            {
              parts: [
                { text: `${SYSTEM_PROMPT}\n\nAnalyze the attached contract titled "${contract.filename}" and return the JSON described in the system prompt. Be thorough but return valid JSON only.` },
                mediaBlock,
              ],
            },
          ],
      ...
      return { run_id: runRef.id, status: "completed" as const };
    } catch (err) {
